...
class MyAPI extends API {
    ...
    protected function users( $val, $data ) {
    	if ($this->method == 'GET') {
    		if (!empty($val[0])) $where = "id={$val[0]}"; else $where = '';
    		return _query('users', $where);
    	} else if ($this->method == 'POST'){
    		$user_col = array('user', 'pass', 'email');
    		foreach ($data as $k => $v) {
    			if ( in_array($k, $user_col) ) $d[$k] = $v;
    		}
    		return _insert('users', $d);
            ...
            return _update('users', $d, $where);
        } else if ($this->method == 'DELETE'){ 
            if (!empty($val[0])) $d = array( "id"=>$val[0]);
            return _delete('users', $d);
        }
    }
